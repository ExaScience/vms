...
{
	const int features_per_memory_read = 32; // 512 / sizeof(F_base)
	for (int i = 0; i < num_compounds; ++i)
	{
#pragma HLS loop_tripcount min = block_size max = block_size
		for (int d = 0; d < num_features; d += features_per_memory_read )
			for (int e = 0; e < features_per_memory_read; e++)
			{
#pragma HLS LOOP_FLATTEN
#pragma HLS PIPELINE II = 1
				if (d+e < num_features)
					features_stream << features[i][d+e];
			}
	}
}
