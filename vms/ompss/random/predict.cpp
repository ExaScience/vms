...
{
    static const F_base  empty_in[num_features] = {0};
    static       P_base  empty_out[num_proteins];

    predict_or_update_model(true, empty_in, empty_out, &U[0][0][0], &mu[0][0], &B[0][0][0]);
#pragma omp taskwait
}
...
)
{
    ...
    predict_or_update_model(false, in, out, &empty_U[0][0][0], &empty_mu[0][0], &empty_B[0][0][0]);
}
